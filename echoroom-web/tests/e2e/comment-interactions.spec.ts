...
test.describe("Comment interactions", () => {
  ...
  test("Comment submit button disabled when input empty", () => {
    ...
    const hasDisabledLogic = source.includes("disabled") && (source.includes("length") || source.includes("empty") || source.includes("trim"));
    if (!hasDisabledLogic) {
      test.info().annotations.push({ type: "info", description: "No explicit disabled state found for comment submit" });
    }
  });
  ...
  test("Scenario detail has comment input if authenticated", () => {
    ...
    const hasCommentElements = source.includes("form") || source.includes("textarea") || source.includes("input") || source.includes("comment") || source.includes("Commentaire");
    if (!hasCommentElements) {
      test.info().annotations.push({ type: "info", description: "No comment form/input found in scenario detail page" });
    }
  });
  ...
  test("Non-authenticated users see login prompt on scenario detail", () => {
    ...
    const hasLoginPrompt = source.includes("Connectez-vous") || source.includes("login") || source.includes("connexion");
    if (!hasLoginPrompt) {
      test.info().annotations.push({ type: "info", description: "No explicit login prompt found in scenario detail comment section" });
    }
  });
  ...
  test("mock: scenario detail comment data loads via API", async ({ page }) => {
    ...
    const headingVisible = await page.getByText("Test Scenario").isVisible().catch(() => false);
    if (headingVisible) {
