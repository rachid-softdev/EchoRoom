...
test.describe("Leaderboard page", () => {
  ...
  test("route /leaderboard exists and responds with a redirect (status < 400, not 404)", async ({ page }) => {
  ...
  test("DashboardShell title 'Classement' is visible when authenticated", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Classement" }),
    ).toBeVisible();
  });
  ...
  test("tab buttons 'Scénarios' and 'Créateurs' are visible when authenticated", async ({ page }) => {
    await page.goto("/leaderboard");
    await page.waitForLoadState("networkidle");
    ...
    await expect(
      page.getByRole("button", { name: "Créateurs" }),
    ).toBeVisible();
  });

  test("period filter buttons 'Tout', 'Cette semaine', 'Ce mois' are visible when authenticated", async ({ page }) => {
    await page.goto("/leaderboard");
    await page.waitForLoadState("networkidle");
    ...
    await expect(
      page.getByRole("button", { name: "Tout" }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: "Cette semaine" }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: "Ce mois" }),
    ).toBeVisible();
  });
