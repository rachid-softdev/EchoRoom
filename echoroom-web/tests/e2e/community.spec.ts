...
test.describe("Community page", () => {
  ...
  test("route /community exists and responds with a redirect (status < 400, not 404)", async ({ page }) => {
  ...
  test("DashboardShell title 'Communauté' is visible when authenticated", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Communauté" }),
    ).toBeVisible();
  });
  ...
  test("subtitle text is visible when authenticated", async ({ page }) => {
    ...
    await expect(
      page.getByText("Les meilleurs moments partagés par la communauté"),
    ).toBeVisible();
  });
  ...
  test("comment input placeholder 'Ajouter un commentaire...' is visible when data loaded", async ({ page }) => {
    await page.goto("/community");
    await page.waitForLoadState("networkidle");
    ...
    await expect(
      page.getByPlaceholder("Ajouter un commentaire..."),
    ).toBeVisible();
  });
  ...
  test("comment send button is visible when data loaded", async ({ page }) => {
    ...
    const sendButton = page.getByPlaceholder("Ajouter un commentaire...").locator("xpath=../button");
