...
import { getR2Key, R2_BUCKET, R2_PUBLIC_URL, r2Client } from "@/lib/r2";
...
export async function getAudioStream(storedUrl: string): Promise<ReadableStream | null> {
  try {
    const key = getR2Key(storedUrl);
    if (!key) return null;
