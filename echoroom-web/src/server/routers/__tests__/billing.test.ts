...
// ---------------------------------------------------------------------------
// billingRouter — tRPC router tests
// ---------------------------------------------------------------------------
// getCredits:
//   - UserBilling exists → returns credits
//   - UserBilling inexistant → fallback 0
//
// createCheckout:
//   - Validation Zod (credits min1/max10000, priceId string)
//   - Fallback APP_URL (NEXT_PUBLIC_APP_URL non défini → localhost:3000)
...
describe("billingRouter.getCredits", () => {
  ...
  beforeEach(async () => {
    ...
    handler = billingRouter.getCredits.handler;
  });
...
describe("billingRouter.createCheckout", () => {
  ...
  beforeEach(async () => {
    ...
    const { billingRouter } = await import("../billing");
    // @ts-expect-error — handler captured via mock
    handler = billingRouter.createCheckout.handler;
  });
  ...
  it("should reject credits below minimum (Zod schema min=1)", async () => {
    // Replicate the exact Zod schema used in the router to verify validation logic
