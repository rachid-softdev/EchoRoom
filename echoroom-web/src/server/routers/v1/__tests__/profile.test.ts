...
// ---------------------------------------------------------------------------
// Helper: updateProfile input schema
// ---------------------------------------------------------------------------
...
// me — profile query
// ---------------------------------------------------------------------------
describe("profileV1Router.me — current user profile query", () => {
  let mockDb: any;
  ...
  beforeEach(async () => {
    ...
    const { profileV1Router } = await import("../profile");
    // @ts-expect-error — handler captured at import time via tRPC mock
    handler = profileV1Router.me.handler;
  });
  ...
  it("should handle null-safe fields correctly", async () => {
    mockDb.user.findUnique.mockResolvedValue({
      id: "user-123",
      email: "test@example.com",
      username: "testuser",
  ...
  it("should throw NOT_FOUND when user does not exist", async () => {
    ...
    await expect(handler({ ctx: validCtx })).rejects.toThrow(TRPCError);
...
// ---------------------------------------------------------------------------
describe("profileV1Router.updateProfile — username update", () => {
  let mockDb: any;
  let handler: Function;
  const validCtx = { session: { user: { id: "user-123" } } };
  ...
  beforeEach(async () => {
    ...
    const { profileV1Router } = await import("../profile");
    // @ts-expect-error
    handler = profileV1Router.updateProfile.handler;
  });
...
// ---------------------------------------------------------------------------
describe("profileV1Router.exportData — GDPR data portability", () => {
  let mockDb: any;
  let handler: Function;
  const validCtx = { session: { user: { id: "user-123" } } };
  ...
  beforeEach(async () => {
    ...
    const { profileV1Router } = await import("../profile");
    // @ts-expect-error
    handler = profileV1Router.exportData.handler;
  });
...
// ---------------------------------------------------------------------------
// deleteMyAccount — GDPR account deletion
// ---------------------------------------------------------------------------
describe("profileV1Router.deleteMyAccount — GDPR account deletion", () => {
  let mockDb: any;
  let handler: Function;
  const validCtx = { session: { user: { id: "user-123" } } };
