...
export function ClipCreator({ scenarioId }: ClipCreatorProps) {
  ...
  const canCreate = selectedCallId && clipEndTime > clipStartTime && clipEndTime <= maxDuration && !createClipMutation.isPending
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                value={clipStartTime}
                onChange={(e) => setClipStartTime(Math.max(0, Math.min(Number(e.target.value), maxDuration - 1)))}
                placeholder="0"
              />
            </div>
            <div className="space-y-2">
              ...
              <Input
                ...
                value={clipEndTime}
                onChange={(e) => setClipEndTime(Math.max(1, Math.min(Number(e.target.value), maxDuration)))}
                placeholder="30"
              />
            </div>
          </div>
